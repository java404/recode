...
echo "Dropping database (${_db_host}:${_db_port},${_db_user}) smartmon_core service."

mysql -h ${_db_host} -P ${_db_port} -u${_db_user} -p${_db_password} \
  -e "DROP DATABASE IF EXISTS smartmon_core; "
_sql_result=$?

if [ ${_sql_result} -ne 0 ]; then
  echo "ERROR: SmartMon Core Service DB drop error."
  exit 1
fi

echo "SmartMon Core Service DB is dropped."
exit 0